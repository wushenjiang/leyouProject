    ...
    public SendSmsResponse sendSms(String phoneNumber, String signName, String templateCode, String templateParam) {
        ...
        if (StringUtils.isNotBlank(lastTime)) {
            ...
            if(System.currentTimeMillis() - last < SMS_MIN_INTERVAL_IN_MILLIS){
                log.info("[短信服务] 发送短信频率过高，被拦截，手机号为:{}",phoneNumber);
                return null;
            }
        }
        try {
            ...
            log.error("accessKeyId =" + prop.getAccessKeyId());
            //初始化acsClient,暂不支持region化
            IClientProfile profile = DefaultProfile.getProfile("cn-handan", prop.getAccessKeyId(), prop.getAccessKeySecret());
            DefaultProfile.addEndpoint("cn-handan", product, domain);
